...
const DirectionsMap: React.FC<DirectionsMapProps> = ({ endLocation, endLocationName, mapboxToken }) => {
  ...
  const routeLayerStyle: mapboxgl.LineLayer = { // Use mapboxgl.LineLayer type
    id: 'route',
    type: 'line',
  ...
  const getDirections = useCallback(async () => {
    ...
    try {
      ...
      // Pass addresses/names for origin and destination as the API geocodes them
      ...
      if (data.routes && data.routes.length > 0 && data.routes[0].geometry) {
        ...
        if (data.origin && data.origin.coordinates) {
          ...
          if (mapRef.current && data.routes[0].geometry?.coordinates) {
             ...
             if (Array.isArray(coordinates) && coordinates.length > 0 && Array.isArray(coordinates[0])) {
                ...
                /* Alternative using reduce starting from the second element:
                const boundsReduce = coordinates.slice(1).reduce(
                    (b: mapboxgl.LngLatBounds, coord: [number, number]) => {
                        return b.extend(coord);
                    },
                    new mapboxgl.LngLatBounds(coordinates[0], coordinates[0]) // Initialize with the first point
                );
                */
           ...
           // Attempt to center map based on destination if start fails
            if (mapRef.current) {
                mapRef.current.flyTo({ center: [endLocation.longitude, endLocation.latitude], zoom: 13 });
            }
        }
  ...
  }, [startLocationInput, endLocationName, endLocation, mode]); // Added endLocation to dependency array
  ...
  useEffect(() => {
    setViewport(prev => ({
     ...
     if (mapRef.current) {
        mapRef.current.flyTo({ center: [endLocation.longitude, endLocation.latitude], zoom: 12 });
     }
